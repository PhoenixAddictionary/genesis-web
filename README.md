...
| `plugin/`, `de/plugin/`, `hu/plugin/`, `tools/render_markdown_page.py` | `PLUG-IN.md` (and its `.de`/`.hu` siblings) rendered into a real styled page instead of raw markdown source — for a visitor following a link from outside GitHub. Not a runtime build step: the generator runs once at commit time and its plain HTML output is committed, same pattern as `build_corpus.py`; CI fails if the committed pages drift from their markdown source. |
